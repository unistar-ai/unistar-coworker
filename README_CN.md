...
`build.rs` **不会**跑 `npm` — 前端构建由开发者、CI 或 `start-agent.sh` 负责（`npm run build:fast`）。启用 `embed-web-ui` 时，manifest 按内容去重，只有 bundle 真变才触发重编译。未启用时需先 `npm run build:fast`，否则 React 路由返回 503（legacy UI 可能仍可用）。
...
**发布 / 部署**（单二进制、内嵌 UI — 与 CI、`start-agent.sh` 一致）：
...
| `embed-web-ui` | off | 编译期将 `web-ui/dist/` 嵌入二进制（`include_str!`）。发布、CI、`start-agent.sh` 启用；本地 `cargo check` / `cargo build` 省略以加速（UI 从磁盘 serve）。 |

vendored 的 `chromiumoxide` 补丁在 `vendor/chromiumoxide/`，用于应对 CDP schema 漂移。

Web UI（`web-ui/`）需要 Node 18+，用 `npm run build:fast` 构建（由开发者 / CI / `start-agent.sh` 负责，**非** `build.rs`）。启用 `embed-web-ui` 时 bundle 编入二进制；未启用时 `serve` 运行时读 `web-ui/dist/`。

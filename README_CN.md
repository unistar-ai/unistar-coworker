...
- GitHub/CI 在 Rust 进程内执行（[`GithubHarness`](./src/github/harness.rs) → `gh` CLI）— GitHub 不依赖 MCP 子进程。
...
Web UI 是 **React 18 SPA**（Vite + Tailwind + Radix UI + zustand），编译期内嵌进二进制。提供流式聊天（含工具/reasoning 卡片）、上下文面板、审批弹窗、主题切换、Markdown 转录导出。源码在 `web-ui/`；`build.rs` 在 `cargo build` 时跑 `vite build` 并通过 `include_str!`/`include_bytes!` 嵌入产物。
...
| **Harness** | `src/agent/`、`src/engine/` | 确定性 Rust — 调度、MCP 池、审批、token 预算、循环 |

工具名 SSOT：[`skills/_base/TOOLS.md`](./skills/_base/TOOLS.md) + [`src/agent/tool_catalog.rs`](./src/agent/tool_catalog.rs)。
...
```bash
# Rust 后端（build.rs 自动跑 web-ui/ 的 vite build）
cargo check
cargo clippy -- -D warnings
cargo test
cargo test --no-default-features   # 不含 headless Chromium 的精简构建
cargo fmt --check
```

若 `npm` 不可用，`cargo build` 跳过前端重建并降级使用已有 `web-ui/dist/`（或 dist 缺失时 `/` 返回 503）。装 Node 来构建 React UI：
...
CI（`.github/workflows/ci.yml`）运行 `cargo fmt --check`、`cargo clippy -D warnings`、`cargo test`、一个 `--no-default-features` 构建/测试 job，以及一个**可选**的 Playwright 冒烟 job（`continue-on-error: true`）。
...
```bash
cargo build                                    # 先构建：target/debug/unistar-coworker
cd web-e2e
npm install
...
| Feature | 默认 | 用途 |
|---------|------|------|
| `web-browser` | on | `web_fetch` 浏览器模式用的 headless Chromium（引入 `chromiumoxide`）。用 `--no-default-features` 禁用可得到更精简的构建，`web_fetch` 回退到纯 HTTP。 |

vendored 的 `chromiumoxide` 补丁在 `vendor/chromiumoxide/`，用于应对 CDP schema 漂移。

Web UI（`web-ui/`）需要 Node 18+，由 `build.rs` 通过 `npm run build:fast` 构建，编译期内嵌。它不是 Cargo feature。
...
```
unistar-coworker/
├── prompts/chat.md          # Chat system prompt（编译期内嵌）
├── skills/                  # 技巧 skill（SKILL.md）+ _base/TOOLS.md SSOT
├── src/
│   ├── main.rs              # CLI 入口
│   ├── build.rs             # 跑 vite build，内嵌 web-ui/dist/
│   ├── config.rs            # YAML 配置模型
│   ├── agent/               # Chat 循环、工具目录、上下文、triage
│   ├── engine/              # Engine、调度器、workflow、审批、prompt
│   ├── github/              # GithubHarness（进程内 gh）
│   ├── llm/                 # OpenAI 兼容客户端、流式、classify
│   ├── mcp/                 # 可选 MCP 联邦池
│   ├── store/               # JSON + SQLite 持久化、migrate、compact
│   ├── tui/                 # ratatui 终端 UI
│   ├── web/                 # axum Web 服务 + React UI 内嵌（ui.rs）
│   └── output/              # Digest 导出

...
- 运行 **chat**（TUI、CLI、Web）与定时 **workflow**，通过 skills + prompts 支持编码、问答与运维（`chat.workspace`）。
- 使用 **本地 LLM**（Ollama / OpenAI 兼容）做规划、工具调用与摘要；支持命名 profile 与运行时切换。
- 提供 **工作区工具**（`read_file`、`grep`、`bash_run` 等）；可选 **MCP**（`mcp.servers[]`）。
- 在需要时集成进程内 **GithubHarness**（`gh` CLI）；`daily-work`、`review-radar` 等是已内置的领域包，不是产品上限。
...
| **Workflow** | 可选 `daily-work`、`review-radar`；支持 cron、daemon、单次运行 |
...
`build.rs` **不会**跑 `npm` — 前端构建由开发者、CI 或 [`scripts/package.sh`](./scripts/package.sh) 负责（`npm run build:fast`）。启用 `embed-web-ui` 时，manifest 按内容去重，只有 bundle 真变才触发重编译。未启用时需先 `npm run build:fast`，否则 React 路由返回 503（legacy UI 可能仍可用）。
...
- 保持 `web.bind` 为默认 **`127.0.0.1:8787`** — 聊天、审批、workflow 仅监听回环地址。
...
| 是 | 不是 |
|----|------|
| **本地优先的通用 Agent**（chat + workflow + 工具），面向本地 LLM | 云端托管 Agent 或多租户 SaaS |
...
**刻意不做：** 无托管遥测；无审批自动 merge；Workflow 默认不调用第三方 MCP（Chat 在配置后可调用）。GitHub 运维（`daily-work`、`review-radar` 等）是 **内置领域包**，不是唯一场景。
...
版本：**2.4.1**（workspace `[workspace.package]`，见 [Cargo.toml](./Cargo.toml)）。本地模型：[docs/local-models.md](./docs/local-models.md) · 上下文：[docs/context-budget.md](./docs/context-budget.md)。

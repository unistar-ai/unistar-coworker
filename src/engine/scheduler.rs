...
impl Scheduler {
    pub fn from_config(config: &Config) -> Self {
        ...
        let mut push = |workflow_id: &str, cron_expr: &str, label: &str| {
            ...
            match Schedule::from_str(&normalize_cron(cron_expr)) {
                ...
                Err(e) => warn!("invalid cron `{cron_expr}` for {workflow_id}: {e}"),
            }
        };
    ...
    pub fn spawn(self, engine: Arc<Engine>) {
        if self.jobs.is_empty() {
            info!("scheduler: no cron jobs configured");
            return;
        }
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    if engine.is_busy().await {
                        warn!(
                            "scheduler: skip {} — engine busy",
                            job.workflow_id
                        );
                        continue;
                    }

                    info!("scheduler: running {}", job.workflow_id);
                    if let Err(e) = engine.run_workflow(&job.workflow_id).await {
                        warn!("scheduler: {} failed: {e}", job.workflow_id);
                    }
                }
            });
        }

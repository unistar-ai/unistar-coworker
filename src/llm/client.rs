...
Failed logs (this page only):\n{logs}"
    ...
    /// Multi-turn chat for Chat mode — JSON agent step schema (not classify).
    #[allow(dead_code)]
    pub async fn complete_chat_json(&self, messages: &serde_json::Value) -> Result<String> {
        self.complete_chat_json_with_progress(messages, None, |_, _| {})
            .await
    }

    /// Like [`complete_chat_json`](Self::complete_chat_json) but invokes `on_buffer(content, thinking)` as tokens arrive.
    pub async fn complete_chat_json_with_progress<F>(
        &self,
        messages: &serde_json::Value,
        ...
        F: FnMut(&str, &str) + Send,
    {
        ...
        let schema = super::chat::chat_step_schema();
            ...
            if i > 0 {
                if last.trim().is_empty() {
                    msgs.as_array_mut().unwrap().push(serde_json::json!({
                        "role": "user",
                        "content": "Your last turn returned NO output in message.content. \
Return exactly ONE JSON object — \
{\"action\":\"reply\",\"message\":\"...\"} or \
{\"action\":\"tool\",\"tool_name\":\"...\",\"tool_args\":{...}}. \
Put the JSON in message.content; do not leave it empty."
                    }));
                } else {
                match super::chat::parse_chat_agent_step(&last) {
                    Ok(step)
                        if step.action == super::chat::ChatAgentAction::Reply
                            && super::chat::reply_message_looks_truncated(&step.message) =>
                    {
                        msgs.as_array_mut().unwrap().push(serde_json::json!({
                            "role": "user",
                            "content": format!(
                                "Your previous reply JSON had a truncated `message` field (cut off mid-sentence). \
                                 Return action:reply with the COMPLETE user-facing message — include everything \
                                 you already wrote and finish the thought. Do not start over with a new introduction.\n\n\
                                 Partial message so far:\n{}",
                                step.message
                            )
                        }));
                    }
                    Err(_) => {
                        let preview: String = last.chars().take(400).collect();
                        msgs.as_array_mut().unwrap().push(serde_json::json!({
                            "role": "user",
                            "content": format!(
                                "Your last turn was NOT valid JSON. Return exactly ONE JSON object — \
                                 {{\"action\":\"reply\",\"message\":\"...\"}} or \
                                 {{\"action\":\"tool\",\"tool_name\":\"...\",\"tool_args\":{{...}}}}. \
                                 No markdown fences, no prose outside JSON.\n\n\
                                 Invalid output started with:\n{preview}"
                            )
                        }));
                    }
                    _ => {}
            ...
            let (content, done_reason) = if let Some(ollama_base) =
                ollama_api_base(&self.cfg.base_url)
            {
                match self
                    .chat_ollama_with_schema_stream(
                        &ollama_base,
                        &msgs,
                        think_override,
                        &schema,
                    ...
                    .await
                {
                    ...
                    Err(e) if i + 1 < attempt_count => {
                        tracing::warn!("chat ollama stream failed ({e}); retrying without think");
                        last = String::new();
                        continue;
                    }
                    Err(e) => return Err(e),
                ...
                let (content, done_reason) = self
                    .chat_openai_with_schema(&msgs, &schema, num_predict)
                    .await?;
                on_buffer(&content, "");
                (content, done_reason)
            };
            last = content.clone();
            let needs_retry = super::chat::chat_response_needs_json_retry(
                &content,
            ...
            if needs_retry && i + 1 < attempt_count {
                if content.trim().is_empty() {
                    tracing::warn!(
                        "chat step empty (done_reason={done_reason:?}); retrying without think"
                    );
                } else if super::chat::parse_chat_agent_step(&content).is_err() {
                    tracing::warn!(
                        "chat step not valid JSON ({} chars); retrying without think",
                        content.len()
                    );
                } else {
                    tracing::warn!(
                        "chat JSON truncated (done_reason={done_reason:?}, {} chars); retrying without think",
                        content.len()
                    );
                }
                continue;
            ...
            if content.trim().is_empty() {
                return Err(CoworkerError::Other(anyhow::anyhow!(
                    "ollama returned empty chat JSON after {attempt_count} attempts (done_reason={done_reason:?})"
                )));
            }
            return Ok(content);
        }
        if last.trim().is_empty() {
            return Err(CoworkerError::Other(anyhow::anyhow!(
                "ollama returned empty chat JSON after {attempt_count} attempts"
            )));
        }
        Ok(last)
    }

    /// Multi-turn chat using native `tools` / `tool_calls` (not JSON agent-step schema).
...
Either call exactly one tool or reply to the user in plain text."
    ...
    #[allow(clippy::too_many_arguments)]
    async fn chat_ollama_with_schema_stream<F>(
        &self,
        base: &str,
        messages: &serde_json::Value,
        think_override: Option<bool>,
        ...
        F: FnMut(&str, &str) + Send,
    {
        ...
        while !stop_stream {
            ...
            if stream_started.elapsed() >= stream_wall_limit {
                tracing::warn!(
                    "chat stream wall limit {}s (thinking {} chars, content {} chars)",
            ...
            let chunk = match tokio::time::timeout(idle_timeout, stream.next()).await {
                ...
                Err(_) => {
                    tracing::warn!(
                        "ollama chat stream idle {}s (thinking {} chars, content {} chars)",
            ...
            while let Some(pos) = line_buf.find('\n') {
                ...
                if stream_should_abort_thinking(
                    ...
                    tracing::warn!(
                        "chat stream aborted during thinking (~{} chars, content {} chars)",
                        thinking_full.len(),
                        full.len()
                    );
                    stop_stream = true;
        ...
        let content = finalize_ollama_stream_chat_content(full, &thinking_full, &done_reason);
    ...
    async fn chat_openai_with_schema(
        &self,
        messages: &serde_json::Value,
        schema: &serde_json::Value,
        num_predict: u32,
        ...
        let done_reason = v
            ...
            .and_then(|d| d.as_str())
            .map(str::to_string);
        Ok((extract_openai_chat_content(&v)?, done_reason))
    }
...
fn finalize_ollama_stream_chat_content(
    full: String,
    thinking_full: &str,
    done_reason: &Option<String>,
) -> String {
    let selected = super::chat::select_best_chat_step_text(&full, thinking_full);
    if selected.trim().is_empty() {
        tracing::warn!(
            "ollama stream empty content (done_reason={done_reason:?}, thinking {} chars)",
            thinking_full.len()
        );
    }
    selected
...
mod tests {
    ...
    #[test]
    fn finalize_stream_chat_content_salvages_thinking_json() {
        let thinking = "plan...\n{\"action\":\"tool\",\"tool_name\":\"pr_get_overview\",\"tool_args\":{\"repo\":\"o/r\",\"pr_number\":1}}";
        let out = finalize_ollama_stream_chat_content(
            String::new(),
            thinking,
            &Some("stop".into()),
        );
        assert!(out.contains("\"action\":\"tool\""));
        assert!(out.contains("pr_get_overview"));
        assert!(out.contains("\"repo\":\"o/r\""));
    }

    #[test]
    fn finalize_prefers_complete_thinking_over_incomplete_content() {
        let content = r#"{"action":"tool","tool_name":"pr_get_overview","tool_args":{"pr_number":18286}}"#;

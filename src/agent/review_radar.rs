...
use crate::agent::parse::{ci_is_passing, is_review_required, parse_pr_line};
...
pub async fn run_review_radar(
    ...
    for repo in config.repos.clone() {
        log("info", format!("listing open PRs for {repo}"));
        ...
        for line in list_text.lines() {
            ...
            if pr.is_draft || !ci_is_passing(&pr.ci) || !is_review_required(&pr.review) {
                continue;
            }
...
mod tests {
    ...
    fn summary_lists_prs() {
        ...
        assert!(s.contains("Kong/kong-ee#19194"));
        assert!(s.contains("https://github.com/Kong/kong-ee/pull/19194"));
    }

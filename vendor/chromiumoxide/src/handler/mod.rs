...
pub struct Handler {
    ...
    closing: bool,
    /// CDP messages Chrome sends that chromiumoxide_cdp cannot parse (benign drift).
    cdp_invalid_warn_count: u16,
}

impl Handler {
    fn log_invalid_cdp_message(&mut self, payload: &str, serde_err: &serde_json::Error) {
        const WARN_CAP: u16 = 5;
        self.cdp_invalid_warn_count = self.cdp_invalid_warn_count.saturating_add(1);
        let method = serde_json::from_str::<serde_json::Value>(payload)
            .ok()
            .and_then(|v| v.get("method").and_then(|m| m.as_str()).map(str::to_string));
        let preview = if payload.len() > 1200 {
            format!("{}…", &payload[..1200])
        } else {
            payload.to_string()
        };
        if self.cdp_invalid_warn_count <= WARN_CAP {
            tracing::warn!(
        ...
        } else if self.cdp_invalid_warn_count == WARN_CAP + 1 {
            tracing::warn!(
                "WS Invalid message: further CDP parse errors downgraded to debug (set RUST_LOG=chromiumoxide::handler=debug)"
            );
        } else {
            tracing::debug!(
                method = method.as_deref().unwrap_or("?"),
                ...
                match ev {
                    ...
                    Err(err @ CdpError::InvalidMessage(_, _)) => {
                        if pin.config.ignore_invalid_messages {
                            if let CdpError::InvalidMessage(payload, serde_err) = &err {
                                pin.log_invalid_cdp_message(payload, serde_err);
                            }
                        } else {

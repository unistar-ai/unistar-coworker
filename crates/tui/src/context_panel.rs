...
pub fn context_status_spans(th: ThemePalette, state: &AppState) -> Vec<Span<'static>> {
    if state.tab != coworker_core::app::Tab::Chat {
        return Vec::new();
    }
...
fn render_message_content(
    ...
    let content = coworker_core::terminal::sanitize_terminal_output(content);
...
mod tests {
    use super::*;
    use coworker_core::agent::chat_loop::{ContextLine, ContextSnapshot};
    ...
    fn context_panel_shows_skill_section() {
        use coworker_core::agent::chat_loop::ContextSkillBlock;
        let th = theme::ThemePalette::dark();
        let snap = ContextSnapshot {
            turn: 1,
            message_tokens: 200,
...
"#;
        ...
        state.tab = coworker_core::app::Tab::Chat;
    ...
    fn context_tool_log_fits_panel_width() {
        ...
        assert!(
            lines
                .iter()
                .all(|l| { crate::markdown::line_display_width_for_test(l) <= width as usize }),

...
Fields (see [`snapshot.rs`](./snapshot.rs) `WebSnapshot` for the authoritative list):
...
`spawn_event_loop` in [`lib.rs`](./lib.rs) classifies each
...
1. **Rust**: update `WebLivePatch` / `WebChatPatch` in
   [`snapshot.rs`](./snapshot.rs) and the corresponding

...
pub fn read_base_tools() -> String {
    std::fs::read_to_string(crate::repo::resolve_repo_path(BASE_TOOLS_PATH))
...
pub fn load_prompt(path: impl AsRef<Path>) -> Result<PromptSpec> {
    ...
    if super::embedded_prompts::is_bundled_chat_prompt(path) {
        ...
        if let Ok(raw) = std::fs::read_to_string(crate::repo::resolve_repo_path(path)) {
...
pub fn load_markdown_spec(path: impl AsRef<Path>) -> Result<MarkdownSpec> {
    let path = path.as_ref();
    let resolved = crate::repo::resolve_repo_path(path);
